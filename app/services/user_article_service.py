...
class UserArticleService:
    ...
    async def init_user_vocabulary(self, db: AsyncSession, user: User, article_list: list[Article]) -> None:
        ...
        try:
            ...
            logger.debug(f"Initialized {len(user_articles)} user articles for user {user.id}.")
        except Exception as e:
            logger.critical(f"Failed to initialize vocabulary for user {user.id}: {e}")
            return None
           

    async def get_next_article(self, db: AsyncSession, user_id: int, last_article: Optional[Article]) -> Article | None:
        '''
        ...
        try:
            ...
            for user_article in user_vocabulary:
                ...
                if current_weight >= target_weight:
                    next_article = user_article.article
                    logger.debug(f"Selected next article for user {user_id}: Article ID {next_article.id}, Eng. word: '{next_article.eng_word}'")
                    return next_article
        except Exception as e:
            logger.critical(f"Failed to select next article for user {user_id}: {e}")
            return None
    

    async def update_weight(self, db: AsyncSession, user_id: int, artilce_id: int, delta: int) -> None:
        '''
        ...
        try:
            ...
            logger.debug(f"Updated weight for user {user_id}, article {artilce_id}")
        except Exception as e:
            logger.critical(f"Failed to update weight for user {user_id}, article {artilce_id}: {e}")
            return None
